...
const PokeroleEngine = {
    ...
    calculateRank: function(basePkmn, rankIndex) {
        ...
        if (rank >= 4) { this.distribute(attr, 2, 10, maxAttrs); this.distribute(social, 2, 5); this.distribute(skills, 2, 4); }
        if (rank >= 5) { this.distribute(attr, 2, 10, maxAttrs); this.distribute(social, 2, 5); this.distribute(skills, 1, 5); }
        if (rank >= 6) { 
        ...
        if (rank >= 7) { this.distribute(attr, 14, 10, maxAttrs, 2); this.distribute(skills, 1, 5); }

        return {
            ...p, attr, skills, social,
